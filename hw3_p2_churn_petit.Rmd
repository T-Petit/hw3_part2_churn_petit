...
class_train_tree1, churn_train$Exited, positive="1")
...
> The accuracy on the training dataset is 85.65% compared to 85.26% on the test set indicating both models are pretty accurate. Sensitivity is 45.4% on the training set and 43.6% on the test set indicating the model does a poor job predicting 1's on both sets. Although the sensitivity went down slightly on the test data set, overfitting isn't a concern as a little decrease in performance is expected on the test data. Specificity is 96.2% on both the training and test sets indicating the model does an excellent job classifying observations as 0's on both data sets. Overall, like the logistic regression model, the simple decision tree is good at predicting 0's but not at predicting 1's. 
