...
> I decided to convert both `Geography` and `Gender` into factors because these to variables make more sense as factors than characters. `Gender` has two distinct categories and `Geography` has three distinct categories and categorical data makes more sense as factors than as character data. I also decided to conver `HasCrCard`, `IsActiveMember`, and `Exited` since these are binary variables which are categorical and make more sense as Factors.
